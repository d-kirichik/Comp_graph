...
void calcTorus(struct point** vertices) {
	double phi = 2*M_PI/partition, theta = 2*M_PI/partition;
	int i, j;
	for (j = 0; j <= partition; j++) {
		for (i = 0; i <= partition; i++) {
            vertices[j][i].x = (TORUS_MAJOR + TORUS_MINOR * cos(theta*i)) * cos(phi*j);
            vertices[j][i].y = (TORUS_MAJOR + TORUS_MINOR * cos(theta*i)) * sin(phi*j);
            vertices[j][i].z = TORUS_MINOR * sin(theta*i);		
        }
	}
}
...
void drawTorus() {
    int i;
	struct point** vertices = (struct point**)malloc((partition+1)*sizeof(struct point*));
	...
	for (i = 0; i <= partition; i++) {
		...
		for (j = 0; j <= partition; j++) {
			first_vertex = vertices[i % partition][j];
		...
		first_vertex = vertices[i % partition][partition-1];
